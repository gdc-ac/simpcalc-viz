...
private:
    ...
    unordered_map<string, TokenType> keywords = {
        {"PRINT", TokenType::PRINT}, {"IF", TokenType::IF}, {"ELSE", TokenType::ELSE}, {"ENDIF", TokenType::ENDIF}, {"SQRT", TokenType::SQRT}, {"AND", TokenType::AND}, {"OR", TokenType::OR}, {"NOT", TokenType::NOT}};
    ...
    // Constructor which takes in a string as input
public:
    Scanner(const string &src) : input(src) {}

    // Gets a singular token
    Token getToken()
    {
    ...
    // All results of getToken() are stored in a vector of tokens called scanAll.
...
// We define a Parser as a struct that takes in tokens and checks if the input file is a valid SimpCalc program.
class Parser
{
private:
    vector<Token> tokens;
    ...
    // Matching with the needed token
    ...
    // The following void functions below implements all productions of the SimpCalc grammar and check for any syntax errors.
    void Prg()
    {
        Blk();
        if (!errorFlag)
    ...
    void Blk()
    {
        while (!errorFlag && (current().type == TokenType::IDENTIFIER ||
    ...
    void Rel()
    {
        ...
        case TokenType::GREATER_THAN:
            ...
            break;
        case TokenType::GT_EQUAL:
            match(TokenType::GT_EQUAL);
            break;
        case TokenType::NOT_EQUAL:
            match(TokenType::NOT_EQUAL);
            break;
        case TokenType::LT_EQUAL:
            match(TokenType::LT_EQUAL);
            break;
        default:
            outParse << "Missing relational operator\n";
            errorFlag = true;
            break;
        }
    }

    // Constructor that takes in a vector of tokens
public:
...
// The main() function takes in a series of input files, applies the Scanner-Parser, and then saves the output to corresponding text files.
int main()
{
    for (const auto &entry : filesystem::directory_iterator("."))
    {
        if (entry.path().extension() == ".txt")
        {
            ...
            if (!outScan.is_open() || !outParse.is_open())
                continue;
